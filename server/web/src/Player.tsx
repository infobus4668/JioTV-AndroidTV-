...
export function WatchPage() {
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      {/* Orientation-aware: PORTRAIT stacks (player info, then guide) so the 16:9 video uses the full
          width; LANDSCAPE puts the player + guide side-by-side so the short, wide screen is used
          instead of burying the guide below the fold. Width breakpoints alone can't make this call
          (a 812px landscape phone and a 768px portrait tablet overlap), hence `portrait:`/`landscape:`.
          On very narrow landscape phones (<lg) the guide becomes a 300px column; on tablets it grows. */}
      <div className="flex flex-col gap-6 landscape:flex-row landscape:items-start">
        <div className="min-w-0 landscape:flex-1 landscape:min-w-0">
        ...
        <aside className="landscape:w-[300px] landscape:lg:w-[340px] landscape:shrink-0">
...
function PlayerBox({ cid, title }: { cid: string; title: string }) {
  ...
  return (
    <div ref={boxRef} className="player-box relative w-full aspect-video bg-black rounded-lg overflow-hidden"
      onMouseMove={nudge} onMouseLeave={() => setControls(false)}
      // Tap / click anywhere on the video surface (but not on a control) to manage playback:
      // if the controls are hidden a tap brings them up (touch devices have no hover, so this is the
      // only way to summon them again); if they're already visible a tap plays/pauses.
      onClick={(e) => {
      ...
      <div className={`absolute inset-x-0 bottom-0 flex flex-col gap-1.5 p-3 bg-gradient-to-t from-black/80 to-transparent transition-opacity ${controls ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
        ...
        <input type="range" min={0} max={1} step={0.05} value={muted ? 0 : volume} onChange={(e) => onVolInput(Number(e.target.value))} className="hidden sm:block w-20 accent-white" title="Volume" />
