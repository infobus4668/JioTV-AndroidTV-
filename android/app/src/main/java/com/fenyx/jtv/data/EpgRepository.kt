...
data class EpgProgram(
    ...
    val startMs: Long,
    val stopMs: Long,
    // Catch-up identifiers from the native Jio EPG (mirrors server/src/jio/epg.ts). Present only
    // for programmes Jio can replay; srno is the hard requirement — without it a past show cannot
    // be resolved, so UIs grey out its replay affordance.
    val srno: String? = null,
    val showId: String? = null,
    val showtime: String? = null,
    val catchupAvailable: Boolean = false
) {
    /** True when this programme is in the past AND carries everything needed to start a replay. */
    val isReplayable: Boolean
        get() = stopMs > 0 && startMs > 0 && !srno.isNullOrBlank() &&
            (catchupAvailable || System.currentTimeMillis() > stopMs)
}
...
class EpgRepository(private val context: Context) {
    ...
    companion object {
        ...
        /**
         * Clips a programme's [startMs, stopMs) range to the visible [windowStartMs, windowEndMs)
         * guide window. Returns null when the programme doesn't overlap the window at all — the
         * pure geometry behind the EPG time-grid's block layout (unit-tested).
         */
        fun clipProgramToWindow(
            startMs: Long,
            stopMs: Long,
            windowStartMs: Long,
            windowEndMs: Long
        ): LongRange? {
            if (stopMs <= windowStartMs || startMs >= windowEndMs) return null
            val s = startMs.coerceAtLeast(windowStartMs)
            val e = stopMs.coerceAtMost(windowEndMs)
            if (e <= s) return null
            return s until e
        }

        /**
         * Pure parser for Jio's native per-channel EPG JSON (`getepg/get`). Extracted from the
         * network method so the field mapping — including the catch-up identifiers this feature
         * depends on — is unit-testable. Mirrors server/src/jio/epg.ts exactly.
         */
        internal fun parseNativeEpg(json: org.json.JSONObject): List<EpgProgram> {
            val epgArray = json.optJSONArray("epg") ?: return emptyList()
                ...
                if (title.isNotEmpty() && startMs > 0 && stopMs > 0) {
                    programs.add(
                        EpgProgram(
                            ...
                            stopMs = stopMs,
                            srno = obj.optSrno(),
                            showId = obj.optString("showId", "").ifBlank { null },
                            showtime = obj.optString("showtime", "").ifBlank { null },
                            catchupAvailable = obj.optBoolean("isCatchupAvailable", false)
                        )
                    )
                }
            }
        ...
        /** srno arrives as either a JSON number or string depending on the channel. */
        private fun org.json.JSONObject.optSrno(): String? =
            when {
                has("srno") -> optString("srno", "").ifBlank { null }
                else -> null
            }
    }
