...
object JioExoPlayerFactory {
    ...
    fun create(
        ...
        return ExoPlayer.Builder(context, renderersFactory)
            ...
            // ─── System audio integration (phones) ───
            // handleAudioFocus=true: ExoPlayer pauses for phone calls/alarms/other apps' audio and
            // resumes when focus returns. handleAudioBecomingNoisy=true: pause when headphones/BT
            // disconnect instead of blasting the speaker. Both are no-ops on TVs (no calls, no
            // unpluggable headphones), so they're safe to enable unconditionally.
            .setAudioAttributes(
